...
{
    public class CrimeSSPRepository : GenericRepository<CrimeSSP>, ICrimeSSPRepository
    {
        private SafeSpotContext contexto = null;

        public CrimeSSPRepository(SafeSpotContext _Contexto) : base(_Contexto)
        {
            contexto = _Contexto;
        }
        ...
        public List<TipoCrimePorMes_Anual> RetornaTipoCrimePorMes_Anual(Endereco endereco)
        {
            ...
            var resultado = RetornaCrimeSSP(endereco);
        ...
        public String GeraMsgSeguranca(Endereco endereco)
        {
            ...
            if(resultado.Count > 0)
            {
                ...
                var retornoQuery = retorno
                                .GroupBy(_ => _.Bairro)
                                .Select(_ => new {
                                    Bairro = _.FirstOrDefault().Bairro,
                                    QuantidadeCrime = _.Count()
                                });
                ...
                int mediaCrimes = (mediaBairros / qtdBairros);

                if (bairroSelecionado < mediaCrimes)
                    return "A localidade pesquisa é segura.";
                else if (bairroSelecionado == mediaCrimes)
        ...
        public List<CrimeSSP> RetornaCrimeSSP(Endereco endereco)
        {

            List<CrimeSSP> listaCrimes = new List<CrimeSSP>();

            if(endereco.Bairro != null && endereco.CidadeBO != null && endereco.Logradouro == null)
            {
                // Busca por Cidade e Bairro                
                listaCrimes = this.contexto.CrimeSSP.AsNoTracking().Where(c => c.Endereco.CidadeBO.Contains(endereco.CidadeBO.ToUpper())).ToList();
            }
            else if (endereco.Bairro == null && endereco.CidadeBO == null)
            {
                // Busca por Logradouro                
                listaCrimes = this.contexto.CrimeSSP.AsNoTracking().Where(c => c.Endereco.Logradouro.Contains(endereco.Logradouro.ToUpper())).ToList();

            }
            else if (endereco.CidadeBO == null && endereco.Logradouro == null)
            {
                // Busca por Bairro
                listaCrimes = this.contexto.CrimeSSP.AsNoTracking().Where(c => c.Endereco.Bairro.Contains(endereco.Bairro.ToUpper())).ToList();
            ...
            {
                // Busca por Cidade e Bairro
                listaCrimes = this.contexto.CrimeSSP.AsNoTracking().Where(c => c.Endereco.CidadeBO.Contains(endereco.CidadeBO.ToUpper())).Where(c => c.Endereco.Bairro.Contains(endereco.Bairro.ToUpper())).ToList();

            }
            else if (endereco.Bairro == null)
            {
                // Busca por Cidade e Logradouro
                listaCrimes = this.contexto.CrimeSSP.AsNoTracking().Where(c => c.Endereco.CidadeBO.Contains(endereco.CidadeBO.ToUpper())).Where(c => c.Endereco.Logradouro.Contains(endereco.Logradouro.ToUpper())).ToList();

            }
            else
            {
                // Busca por Bairro e Endereço
                listaCrimes = this.contexto.CrimeSSP.AsNoTracking().Where(c => c.Endereco.Bairro.Contains(endereco.Bairro.ToUpper())).Where(c => c.Endereco.Logradouro.Contains(endereco.Logradouro.ToUpper())).ToList();
